...
- **Dateline-Safe Route Rendering**: New `WeatherRoutePainter._unwrapForCamera` + `_unwrapNear` shift the route polyline and the snap-original points into the world copy nearest the camera. Polyline, waypoint rings, chevrons, wind arrows, start / end pins, snap bridges, and the selection halo all stay aligned, so routes crossing the antimeridian (±180°) paint as one continuous polyline at any pan instead of streaking across the canvas.
...
- New `_unwrapForCamera(List<List<double>>, double centerLon)` + `_unwrapNear(List<double>?, double anchorLon)` in `weather_routing_overlay.dart` — pure functions, paint-time hot path.
