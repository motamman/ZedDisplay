...
### Added
- **Crew Intercom & Calls — Multi-Peer Mesh Rewrite**: The WebRTC voice layer (`intercom_service.dart`) was rebuilt around one unified `_PeerSession` per remote peer, replacing the tangle of a single shared transmit peer-connection plus parallel renderer/stream/candidate maps. **Channel PTT now reaches every listener** (3+ included) — a broadcast opens a targeted offer to each online crew member (`data.targetId`) instead of a single connection that only one person heard; a peer that joins mid-broadcast gets its own offer. Direct 1:1 calls run through the same machinery. Everything stays **LAN-only by design** (`iceServers: []`, host candidates — no STUN/TURN).
- **Bulk Alert Actions**: When more than one alert is active, the alert panel shows a header with the count plus **ACK ALL** and **DISMISS ALL** (`AlertCoordinator.acknowledgeAll()` / `dismissAll()`).
- **Always-Visible Screen Selector with Edge Arrows**: The dashboard's screen selector no longer auto-hides — the page dots are always shown, with a prev chevron pinned to the far-left edge and a next chevron at the far-right edge (paging through the infinite-scroll screen list). Tapping the dots still opens the full selector.

### Changed
- **Unified Alert / Notification Panel**: In-app alerts now rise from the area **above the screen-nav row** (clearing both the bottom safe-area inset and the nav height) so they never overlap navigation. The panel re-renders **live** when notification settings change, and `activeAlerts` honors the user's per-level in-app notification filter literally — a severity toggled off in Settings is hidden from the panel even if it's an alarm/emergency (audio and system/crew delivery are gated separately and unaffected). Row actions are unified to a universal **ACK** and **DISMISS** at every severity (color carries the severity), and **VIEW** uses one resolution rule for every alert (see below).
- **Notification Routing by Path — No Mapping Table**: The hardcoded `_keyPrefixMapping` category table (`navigation` → compass/autopilot/…, `electrical` → victron_flow/…, etc.) is gone. SignalK notifications now route to whichever widget the **user bound to the alerting path** — the path is the binding, derived from each tool's own `requiredPaths`. App-internal notifications with no path (crew message, intercom, alarm) still map to their one obvious home widget. VIEW renders nothing (rather than pointing nowhere) when no relevant widget is placed.
- **Power Flow Battery — Vertical Fill**: The state-of-charge visualization changed from a left→right "fuel gauge" to a bottom→top **battery gauge**: the box fills with blue from the bottom up to the SOC fraction, and the remainder above is transparent so the bare container background shows through (visible, but not filled). The fill gradient is layered over the box's normal background rather than replacing it.

### Fixed
- **"Connected but No Data" After Sleep / Server Switch (root cause)**: Device logs showed the server (or an intermediary proxy) was not returning WebSocket pongs, so the client's 30s `pingInterval` tore the socket down every ~60s in a permanent reconnect loop. The client-side ping is now **disabled** — liveness is governed by the watchdog plus the server's own ping/terminate. A monotonic **socket generation** is captured by every stream handler so a torn-down socket's late `onDone`/`onError`/message can no longer clobber the live socket (the core "had to restart to get data back" fix). Reconnects are **single-flighted** so the four unsynchronized callers (backoff timer, background probe, liveness watchdog, wake-from-sleep) can't race and orphan a fresh socket. A lightweight reconnect **no longer wipes the data cache** — with `policy:instant` the server only re-sends a value when it next changes, so clearing left the dashboard blank until each value happened to update; last-known values now stay visible while fresh deltas overwrite them. A WebSocket **error** now drives the same teardown-and-recover path as `onDone` instead of stranding the app "disconnected" with a live orphan socket.
- **AIS / CPA Alerts Now Navigate**: The AIS favorites "in range" alert declared `alarmSource: 'ais_favorites'` — not a real tool type, so VIEW resolved to nothing; it now points at `ais_polar_chart`. Server closest-point-of-approach notifications (`navigation.closestApproach.<vessel>`) had no path-bound widget, so VIEW was hidden; the AIS Polar Chart now declares `navigation.closestApproach.*` in its `requiredPaths` so CPA alerts route to it through the normal path-match.

### Developer
- **Connection-Lifecycle Diagnostics**: `SignalKService` keeps a bounded (200-line) connection-lifecycle ring buffer (`connectionLog`), written by `_log()` and mirrored into `DiagnosticService` for remote post-mortem; app pause/resume transitions are stamped via `logLifecycle()`.
- **Injectable Socket Factory + Reconnect Tests**: A `ChannelFactory` typedef lets tests drive the socket-swap machinery with a fake channel (production path unchanged), with `@visibleForTesting` hooks (`debugSeedConnected`, `debugReconnectLight`, `debugCheckLiveness`, generation/channel getters). New `test/signalk_reconnect_test.dart` (223 lines) exercises the generation guard, single-flight coalescing, and cache-preserving light reconnect; `widget_test.dart` and `weather_routing_itinerary_card_test.dart` repaired.
- **`DashboardService.findScreenWithToolPath(skPath)`**: Resolves a screen by exact-or-dot-boundary path match against each placed tool's `requiredPaths`, preferring the current screen; backs the table-free notification routing.
...
- **Chart Plotter — Overlays/Sheets Dismiss Reliably**: The chart-plotter bottom sheets (vessel detail, AIS list, routes, weather routing, download — and the AIS Polar Chart's vessel list) used `DraggableScrollableSheet` without `expand: false`, so the sheet filled the viewport and **covered the modal barrier** — tapping outside did nothing and swipe-down fought a full-height sheet. Adding `expand: false` exposes the barrier (tap-outside dismisses) and makes swipe-down reliable. The chart plotter's AIS/Routes/Layers nav drawers also gain an explicit full-screen tap-catcher to close on an outside tap (`TapRegion.onTapOutside` never fired because `flutter_map` consumes the tap).
- **Switching SignalK Servers No Longer Needs an App Restart**: `connect()` only tore down the previous connection (cancel auto-reconnect timer + background probe, clear caches) when a live connection still existed. Switching from a server whose socket had already dropped — e.g. leaving a local network for an internet server — skipped that, so the old server's reconnect loop kept firing `_reconnectLight()` against the new connection (forcing an app restart) and stale data / AIS vessels lingered. `connect()` now cancels the previous server's reconnect machinery and wipes its cached state on a **server change**, while leaving a **same-server reconnect (wake-from-sleep) untouched**; a failed connect also resets `_intentionalDisconnect` so it can never strand reconnection.
- **Route Planner Boats — Build-Phase Exception / Frame Drops**: The chart plotter primed the boat/polar list from `didChangeDependencies`, where `RoutePlannerBoatsService.refreshAllBoats()` calls `notifyListeners()` synchronously during the build phase → "setState()/markNeedsBuild() called during build" and dropped frames. The prime now runs in a post-frame callback.
- **Persisted vs Live Required Paths**: `ToolService.createTool` derives `Tool.requiredPaths` via `ToolRegistry().requiredPaths(...)` instead of `config.dataSources` only, so a customProperties-based tool (e.g. Power Flow) persists the same paths it subscribes to (fixes `Tool.isCompatible` seeing an empty list).
- **Power Flow Legacy Battery Config**: The single-`battery`-map backward-compat migration now keeps a pre-0.6.7 config that set voltage/current/etc. but no SOC path (new `BatteryConfig.hasAnyPath`), instead of discarding it for the default house battery.

### Build
- **Bookworm (Debian 12) Linux Build**: New `linux-bookworm-release.yml` builds the Linux app inside a `debian:bookworm` container (glibc 2.36 / GLIBCXX 3.4.30) for `x64` and `arm64`, producing binaries that load on Raspberry Pi OS / Debian-stable systems. The main `linux-release.yml` (Ubuntu 24.04, glibc 2.39) produced binaries that failed on Bookworm with `GLIBC_2.38` / `GLIBCXX_3.4.32 not found` (from `libflutter_webrtc_plugin.so`). `-bookworm` tags are excluded from the main workflow so they trigger only the container build.
...
### Added
- **Re-Runnable Recent Routes / Editable Loaded Pins**: New `WeatherRoutingService.adoptResultAsPlanned(WeatherRouteResult)` rehydrates a computed result's start / end / vias as live planning pins so a loaded route becomes editable and re-composable. Reload a recent route → drag the start, drag a via, hit Compute again — no need to retype the inputs. When the router relocated the endpoint for navigability (`*_original`), the user-intent location seeds the start / end pin (not the router's relocated anchor). An empty result clears the pins so a subsequent re-run can't resubmit the previously-loaded geometry by accident.
- **SI Routing-Tolerances Persistence**: New `_RoutingTolerances` record (sail threshold m/s, tack-penalty s, iso-step s, land-buffer cells, UKC m, shore-step m, simplify m, precision, arrival-radius m) backs the `weather_routing_tolerances` Hive key. All values are SI on disk and on the wire; sliders do the SI ↔ display conversion at the widget boundary. Legacy display-unit keys (`sailThreshKts`, `isoStepMin`) auto-migrate to SI on read so older blobs round-trip without losing values, and the next save unifies to the SI keys.
- **Recent-Route Re-Run**: Reload from the Recent tab and the panel restores both the previously-saved tolerances and the planning pins. Compose-tab Compute and Recent-tab re-run share a single `_buildWeatherRouteRequest(...)` helper so the two paths never drift; foreign-boat re-runs (boats from the shared `/boats/all` inventory that the server 404s on cross-owner `boat_id` shortcuts) expand explicit `polar` + `vessel` overrides automatically.
- **Editable Planning Pins**: Start / end / via markers are now draggable on the chart. Via pins can be deleted from the panel.
- **Virtual START / END Cards for Snap-Relocated Routes**: When the router moves the user's clicked start / end to clear water, the user's original point gets its own card in the itinerary, navigable like a real waypoint. Card shows the snap-adjustment distance with a "Start/End adjusted X to clear water" banner.
- **Dateline-Safe Route Rendering**: New `unwrapLonForCamera` / `unwrapLonNear` helpers in `lib/utils/antimeridian.dart` shift the route polyline and the snap-original points into the world copy nearest the camera. Polyline, waypoint rings, chevrons, wind arrows, start / end pins, snap bridges, and the selection halo all stay aligned, so routes crossing the antimeridian (±180°) paint as one continuous polyline at any pan instead of streaking across the canvas. Chart-plotter route hit-testing uses the same unwrap so waypoints stay tappable on antimeridian-crossing routes.

### Changed
- **Snap-Adjustment Banner — Unit-Aware**: The "Start/End adjusted X to clear water" text now formats through `distanceMd.formatOrRaw(...)` so it honours the user's distance preference (nm / ft / km / m) instead of hardcoded meters.
- **Wave Height Falls Back to `depth` Category**: Both the weather-routing itinerary card's "Waves" row and the wave-heatmap legend now route through `MetadataStore.getByCategory('depth')` when the path-specific `environment.water.waves.height` metadata isn't present (it usually isn't — the value is computed by the router, not pushed via SignalK). Depth and wave height share the same SI base unit (meters), so a user whose depth preference is feet sees waves in feet too instead of silent raw-SI meters. The `'height'` category that the old fallback tried doesn't exist on SignalK servers.
- **Selection Halo Follows Snap-Aware Pin**: When the virtual START / END cards (sentinel indices `-1` / `waypoints.length`) are selected, the on-map cyan halo now paints at the snap-original location — where the user clicked — instead of nothing. Real-waypoint indices unchanged.
- **Virtual END Sentinel Is `waypoints.length` (Not `coords.length`)**: With simplified polylines `waypoints.length` can exceed `coords.length`, in which case the old sentinel collided with real waypoint indices and produced phantom popover cards ("Waypoint 5 of 8" rendering the same final-waypoint payload three times) plus crashes on `s.endOriginal!`. The sentinel now uses the waypoint count, so collisions are impossible and the popover's "Waypoint N of M" matches the addressable waypoint list. Updated across the panel card list, the popover, `_setSelectedWaypoint`, and `_syncWeatherReferenceTime`.
- **Weather Reference Time Honours Virtual Selections**: `_syncWeatherReferenceTime` now treats `-1` and `waypoints.length` as "first / last waypoint time" instead of falling through to departure / now — the weather overlays scrub to the right forecast hour when the user clicks a virtual card.
- **Camera-Pan Uses Waypoint Position**: `_setSelectedWaypoint` now pans to `waypoints[clamped].lon / lat` instead of `coords[clamped]` — the simplified-geometry coord array may not align 1:1 with the waypoint list, so the previous approach could pan to a different position than the selected popover card displayed.
- **AIS Vessel List — Distance Nullable**: `AisVesselListItem.distance` is now nullable. When the chart-plotter host has no own-vessel fix yet, list rows show no distance cell and sort to the bottom of Nearby instead of all showing `0.0 nm` and clustering to the top. Polar-chart host (which always has an own position when rendered) unchanged.
- **AIS List Rebuilds on CPA Alert Service Changes**: The shared list now wraps its body in a `ListenableBuilder` keyed on `cpaAlertService` (when supplied). A swipe-to-dismiss on a CPA-alert row now properly removes the row instead of leaving a stale `Dismissible` that triggered Flutter's "A dismissed Dismissible widget is still part of the tree" assertion.
- **AIS Sheet Listens to Own-Vessel Motion**: The chart plotter's AIS list bottom sheet now subscribes to `Listenable.merge([aisVesselRegistry, _ownVesselTick])` (new `ValueNotifier<int>` bumped from `_refreshVessel`) so distance / CPA / TCPA chips recompute as the boat moves, not just when an AIS delta arrives.
- **Chart Plotter HUD Style / Position — Live**: `_hudStyle` / `_hudPosition` are now getters off `widget.config` instead of `late final` cached fields, so an HUD configurator edit takes effect on the next rebuild (the dashboard keys the tool by toolId, not config, so State survives edits).
- **Tile Server — Self-Throttling Reconfigure**: `_configureTileServer` caches the last-applied `(upstream, token, refresh)` tuple and short-circuits when nothing changed. Hooked into `signalKService` listeners + `didUpdateWidget` so token refresh, upstream change, and `cacheRefresh` config edits all reach the proxy without an app restart. Listener swap on `signalKService` instance change inside `didUpdateWidget` handles the rare constructor-swap case.
- **AIS Vessel List Type Safety**: `alerts` local now typed `Map<String, CpaVesselAlert>` (was an untyped fallthrough from `cpaAlertService.vesselAlerts ?? const {}`); `_buildNearbyRow` parameter matches. Distance / speed formatter strings in the polar-chart adapter now include the space between value and unit (`5.2 nm`, not `5.2nm`) to match the rest of that file.

### Fixed
- **GPS Loss Clears Cached Own-Ship State**: When the GPS fix disappears (lat / lon null on the 1 s tick), the chart plotter now clears `_ownLat / _ownLon / _ownHeading / _ownCog / _ownSog`, ticks `_ownVesselTick`, and reruns `_refreshRulerSnaps()` so any `'self'`-snapped ruler endpoints clear too. Previously the AIS list and detail math kept computing distance / CPA against the last known fix indefinitely.
- **Snap-Banner Overflow**: The "Start/End adjusted X to clear water" `Text` is now wrapped in `Expanded` with `maxLines: 1` + `TextOverflow.ellipsis` so narrow card widths don't blow up the layout.
- **Manual-Add-Favorite Empty Name**: Submitting the Add-Favorite dialog with a valid MMSI but empty name no longer silently dismisses without feedback. New `nameError` state shows "Required" under the Name field; the error clears on edit.
- **AIS Sheet Item Distance Display**: The shared list (and the Favorites tab when a favorited vessel has no fix) skip the distance cell entirely when `distance == null` instead of crashing on `null!` formatting.
- **Adopt-As-Planned on Empty Result**: Calling `adoptResultAsPlanned` with an empty route now clears the planning pins instead of leaving the previously-loaded route's geometry to silently re-submit on the next compose.

### Developer
- New `WeatherRoutingService.adoptResultAsPlanned(WeatherRouteResult)`. Does not call `notifyListeners()` — callers fold it into their own notify (e.g. `loadRecentRoute`).
- New `_RoutingTolerances` typedef + `_defaultRoutingTolerances` const + `_loadRoutingTolerances(StorageService)` + `_buildWeatherRouteRequest(...)` helpers in `weather_routing_panel.dart`, shared by Compose-tab Compute and Recent-tab re-run paths so the two never drift on tolerance interpretation, departure-time semantics, or boat-id-vs-overrides selection.
- New `lib/utils/antimeridian.dart` exporting `unwrapLonForCamera(List<List<double>>, double centerLon)` + `unwrapLonNear(List<double>?, double anchorLon)` — pure functions, used by both `WeatherRoutePainter` (paint-time hot path) and the chart-plotter's tap hit-testing so render and hit-test stay in the same world copy.
- Sentinel virtual indices: `-1` (START) and `waypoints.length` (END). Replaces the old `coords.length` END convention everywhere it was referenced.
...
### Added
- **Chart Plotter — Native Flutter Rendering**: The chart plotter now renders through a pure-Dart S-52 symbology engine (`packages/s52_dart`) into a Flutter `CustomPainter`. No WebView, no JavaScript bridge. OSM basemap via `flutter_map`; S-57 features decoded from MVT vector tiles (`vector_tile` package) and styled with the IHO S-52 Presentation Library. The legacy WebView/OpenLayers chart plotter is deregistered; existing dashboards that reference the old id still load without the V1 widget builder (safe to rebuild). The new plotter reclaims the plain name "Chart Plotter".
- **S-57 Object Visibility Toggles**: New "Chart Object Visibility" section in the chart plotter config. Eight grouped categories (Navigation aids, Depths and hazards, Routing and traffic, Coastline and land, Regulated/administrative, Infrastructure, Administrative regions, Metadata) with ~80 S-57 object classes — each with an on/off switch. Search field filters by 6-letter code (`SUBTLN`) or plain-English label ("submarine transit lane"). Per-group "Show all" / "Hide all" shortcuts. Useful for hiding the high-clutter regulated-area outlines (CATZOC zones, submarine transit lanes, restricted areas) that NOAA packs into US waters.
- **S57TileManager**: New tile-lifecycle service (`lib/services/s57_tile_manager.dart`) with viewport-aware caching, ±1-tile pre-fetch buffer, eviction on zoom/pan, 200 ms leading+trailing throttle on map events, cross-zoom retention (tiles at z±1 kept as fallback during zoom transitions), and a generation counter so the painter repaints reliably on eviction even when the camera hasn't moved. Factored cleanly out of the plotter widget; listens as a `ChangeNotifier`.
- **Cross-tile feature dedup in the tap popover**: features that span tile boundaries (MVT re-emits identical attributes per tile clip) now surface once in the popover, not N times. Signature built from `(objectClass, sorted attributes)`.
- **On-load vessel centering + snap-to-vessel lock**: the map now centers on the vessel as soon as position is known at map-ready (avoids the LI Sound default flashing briefly). When the "Snap to vessel" button is engaged, pan/fling gestures are disabled; pinch-zoom and rotation still work. Tap the button to release.
- **Depth units from MetadataStore**: SOUNDG labels and DEPCNT contour values on the chart now honour the user's depth-unit preset. Reads `depthMeta.symbol` and `depthMeta.convert(1.0)` at engine-build time and passes them through `S52Options`.

### Changed
- **Polygon outlines (LS / LC) now drawn**: the painter previously applied `LS(...)` / `LC(...)` instructions only to line-geometry features. S-52 uses them to draw polygon outlines — ~15 area classes (RESARE edge, EXEZNE, ADMARE, COSARE, HRBARE, FSHGRD, FSHZNE, CHKPNT, M_NSYS, M_SDAT, M_VDAT, M_ACCY, M_HOPA, M_PROD, M_SREL, SWPARE, TWRTPT) now render their intended borders. Painter walks `f.geometry.rings` for both branches and skips nothing on point features (empty collections).
- **Tap hit-test rewrite**: `_featureClosestPoint` replaces the degenerate "first vertex / midpoint" screen-position helper. Line hit-test = perpendicular foot on the nearest segment. Polygon hit-test = **edge proximity only** — interior taps no longer register, so RESARE (covering most of Long Island Sound) can't eclipse nearby buoys. Winner sort is geometry-kind first (point > line > polygon), then S-52 display priority within the same kind. Active-zoom-only filter kills duplicates that cross-zoom retention would otherwise produce.
- **Popover dismissal**: rebuilt around `showModalBottomSheet` with `isScrollControlled: true` and a `BoxConstraints(maxHeight: 50% screen)` sized content block instead of `DraggableScrollableSheet`. Flutter's built-in drag-to-dismiss actually works on the whole sheet now. `barrierColor: Colors.transparent` so the visible map above the sheet stays visually active and tap-outside-to-dismiss feels natural.
- **Cleaner canvas**: attribute-driven labels (`S52Text`, e.g. OBJNAM buoy/light names) and printf-formatted labels (`S52TextFormatted`) are suppressed on the canvas. Tap the feature for the popover. SOUNDG depth soundings (computed by `SOUNDG02`, emitted as `S52TextLiteral`) still render.
- **Paths section hidden**: Chart Plotter's `ConfigSchema` now sets `allowsDataSources: false`. The "Configure Data Sources" card no longer appears for this tool.
- **tool_info.yaml populated** for the new chart plotter: full button-by-button documentation with icon references, data-flow list, feature summary, and upstream-credit section (IHO S-52/S-57, OpenCPN GPL-2.0 attribution with cross-reference to LICENSE-NOTICES and `devdocs/chart-plotter-v3-sprite-report.md`, Freeboard-SK Apache-2.0, flutter_map, vector_tile, CARTO/OSM/Stadia basemap tile sources).

### Fixed
- **Solid black polygons (AC transparency bug)**: `S52InstructionParser` parsed `AC(COLOR,TRANS)` with `args.trim()` — gluing `"ADINF,3"` into a fake colour code that `_resolve` couldn't find and silently fell back to CHBLK. 80 lookups across 5 classes (`marfea`, `hrbfac`, `termnl`, `dnghlt`, `TSEZNE`) were rendering as solid black polygon fills. `_parseAreaColor` now splits on commas, uses `parts[0]` as `colorCode`, parses `parts[1]` as the S-52 transparency level (0–4), and surfaces it via `transparencyAlpha` (255/191/127/63/0). The painter applies the alpha to the fill and skips the draw entirely on level 4.
- **`propertyCompare` JS parity**: `int.tryParse("1,3")` returned null, so comma-list qualifier patterns (e.g. `COLOUR1,3` for red-and-green buoys) never matched numeric feature values. JS `parseInt` parses the leading integer prefix. The Dart version now mirrors that behaviour via a `^-?\d+` regex; 464 comma-list qualifier rows across the production lookup table can match again.
- **Late-arriving tile fetches**: on non-200 responses and in the `catch` path, the tile-manager cached `S57ParsedTile.empty()` but never called `_bumpAndNotify()`. Listeners missed the mutation; UI could stay in a "loading" state indefinitely on a 404. Both failure paths now notify.
- **Hardcoded lookup-kind for popover priority**: the manager's popover-row lookup used a hardcoded `simplified`/`lines`/`symbolisedBoundaries` switch, which would diverge from the engine's actual choice when `S52Options.graphicsStyle` (paper/simplified) or `.boundaries` (plain/symbolised) change. Promoted `S52StyleEngine._preferredTableKind` to public `preferredTableKind(geom)` and the manager now delegates, keeping popover priority in sync with the engine's styling table.
- **Hive box leak in widget tests**: `ChartTileCacheService.initialize()` opens two Hive boxes (`chart_tile_meta`, `chart_tile_regions`). The tearDown in `test/widget_test.dart` only cleared StorageService — the chart-cache boxes leaked between tests. `tearDown` now calls `chartTileCacheService.dispose()`.

### Removed
- `ChartPlotterBuilder` (V1) is no longer registered in `ToolRegistry`. The `chart_plotter_tool.dart` file stays on disk as a reference; the import in `tool_registry.dart` is replaced with a comment note.
- `_formatText` printf-style helper in the chart painter — unused once `S52TextFormatted` rendering was suppressed.
- `_pointInPolygon` helper in the chart painter — unused once polygon hit-test went edge-only.

### Developer
- **`packages/s52_dart`** (new pure-Dart package): S-52 lookup parser, instruction model + parser, feature model with attribute coercion, colour table, sprite atlas metadata, style engine with CS expansion, and 10 CS procedures (`DEPARE01`, `DEPCNT02`, `LIGHTS05`, `OBSTRN04`, `QUAPOS01`, `RESARE02`/`RESTRN01`, `SLCONS03`, `SOUNDG02`, `TOPMAR01`, `WRECKS02`). 244 unit + fixture-driven tests. SDK floor bumped to Dart `>=3.9.0` to match the lockfile.
- **`S52Options` knobs**: `depthUnit`, `depthConversionFactor`, `displayCategory` now consumable from the widget layer.
- **`devdocs/chart-plotter-v3-sprite-report.md`** — comprehensive report on sprite atlas gaps (~25 classes with missing variants including DAYMAR, RECTRC, DWRTCL, M_QUAL/CATZOC stars), OpenCPN as the only realistic upstream source, GPL-2.0 licensing implications, and a concrete converter plan (`chartsymbols.xml` → `sprite.json`).
- **LICENSE-NOTICES corrected**: OpenCPN `data/s57data/` is GPL-2.0 (the prior "LGPL v2+" claim was wrong). Both `packages/s52_dart/LICENSE-NOTICES` and the sprite report agree.
- **Copilot PR review follow-ups**: all seven comments on PR #68 and six on PR #69 addressed or explicitly dismissed with verification (two `library;` directive warnings and one `r0adkll/upload-google-play@v1` input key warning were Copilot false positives — action.yml lists both `track` and `tracks` as valid inputs).
