...
class _AISPolarChartState extends State<AISPolarChart>
  ...
  /// Adapter from the polar chart's internal `_VesselPoint` to the
  /// shared list's row shape. Distance and CPA / TCPA are already
  /// precomputed on `_VesselPoint`, so this is a straight field copy
  /// — no math, no allocation beyond the new `AisVesselListItem`.
  ais_list.AisVesselListItem _vesselToListItem(_VesselPoint v) {
    return ais_list.AisVesselListItem(
      mmsi: v.mmsi,
      name: v.name,
  ...
  Widget _buildVesselList(BuildContext context, bool isDark) {
    return ais_list.AisVesselList(
      vessels:
          _displayVessels.map(_vesselToListItem).toList(growable: false),
