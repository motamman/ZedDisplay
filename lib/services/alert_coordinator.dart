...
class AlertCoordinator extends ChangeNotifier {
  ...
  void submitAlert(AlertEvent event) {
    ...
    // --- In-app snackbar ---
    // Suppressed when master is OFF or when subsystem has an overlay visible
    if (event.wantsInAppSnackbar && masterOn &&
  ...
  bool _getCrewBroadcastAllowed(AlertEvent event) {
    switch (event.subsystem) {
      ...
      default:
        // Other subsystems use per-subsystem crew broadcast prefs
