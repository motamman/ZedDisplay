...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Widget _buildCpaAlertSection(BuildContext context) {
    final cpaService = Provider.of<CpaAlertService>(context);
    final config = cpaService.config;
      ...
      child: Column(
        children: [
          SwitchListTile(
            ...
            subtitle: const Text(
              'Alert when AIS vessels approach too close',
              style: TextStyle(fontSize: 12),
            ),
              ...
              child: Column(
                ...
                children: [
                  ...
                  ListTile(
                    ...
                    title: const Text('Warning Distance'),
                    subtitle: Text(
                      '${(config.warnThresholdMeters / 1852.0).toStringAsFixed(1)} nm - Trigger warning notification',
                      style: const TextStyle(fontSize: 12),
                    ),
                  ),
                  Slider(
                    value: config.warnThresholdMeters,
                    min: 926.0, // 0.5 nm
                    max: 9260.0, // 5 nm
                    divisions: 18,
                  ...
                  ListTile(
                    contentPadding: EdgeInsets.zero,
                    title: const Text('Alarm Distance'),
                    subtitle: Text(
                      '${(config.alarmThresholdMeters / 1852.0).toStringAsFixed(2)} nm - Trigger audio alarm + crew alert',
                      style: const TextStyle(fontSize: 12),
                    ),
                  ),
                  Slider(
                    value: config.alarmThresholdMeters.clamp(92.6, config.warnThresholdMeters - 185.2),
                    min: 92.6, // 0.05 nm
                    max: (config.warnThresholdMeters - 185.2).clamp(92.6, 9260.0),
                    divisions: 20,
                  ...
                  ListTile(
                    ...
                    subtitle: Text(
                      '${(config.tcpaThresholdSeconds / 60).toStringAsFixed(0)} min - Only alert if CPA within this time',
                      style: const TextStyle(fontSize: 12),
                    ),
                  ),
                  Slider(
                    value: config.tcpaThresholdSeconds,
                    min: 300.0, // 5 min
                    max: 3600.0, // 60 min
                    divisions: 11,
                  ...
                  ListTile(
                    ...
                    trailing: DropdownButton<String>(
                      ...
                      items: CpaAlertService.alarmSounds.keys
                  ...
                  SwitchListTile(
                    ...
                    subtitle: const Text(
                      'Broadcast CPA alerts to crew messaging',
                      style: TextStyle(fontSize: 12),
                    ),
                  ...
                  ListTile(
                    ...
                    subtitle: Text(
                      '${(config.cooldownSeconds / 60).toStringAsFixed(0)} min - Suppress repeat alerts per vessel',
                      style: const TextStyle(fontSize: 12),
                    ),
                  ),
                  Slider(
                  ...
                  if (cpaService.vesselAlerts.isNotEmpty) ...[
                    const SizedBox(height: 12),
                    Container(
                      padding: const EdgeInsets.all(12),
                      decoration: BoxDecoration(
                        color: cpaService.hasActiveAlarm
                      ...
                      child: Row(
                        children: [
                          ...
                          Expanded(
                            child: Text(
                              '${cpaService.vesselAlerts.length} vessel(s) in alert zone',
                              style: TextStyle(
                                color: cpaService.hasActiveAlarm
                                    ? Colors.red
                                    : Colors.orange,
                          ...
                          if (cpaService.hasActiveAlarm)
                            TextButton(
                              onPressed: () => cpaService.acknowledgeAlarm(),
