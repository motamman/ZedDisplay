...
| H2 | **CPA threshold persistence migrated to SI.** Tool config now writes `cpaWarnMeters`, `cpaAlarmMeters`, `maxRangeMeters` (SI, immune to later unit-preference changes). Legacy `*Nm` keys are transparently read and converted at load via `NavigationConstants.readDistanceMeters`; next save through the UI rewrites in SI and the old keys fade out naturally. Verified purely client-side (dashboards persist to local Hive box only). Renamed `AISPolarChart.maxRangeNm` parameter → `maxRangeMeters`; widget now accepts SI directly rather than converting via `_displayToMeters` internally. Updated `tool_config_service` default-factory parameter from `aisMaxRangeNm` → `aisMaxRangeMeters`. Configurator dropdown labels remain in nautical miles (navigation standard) but dropdown values are in meters. Added 5-case migration test covering SI-key preference, NM-key fallback, default, null props, int coercion | `lib/config/navigation_constants.dart` (new helper `readDistanceMeters`), `lib/widgets/tools/ais_polar_chart_tool.dart`, `lib/widgets/ais_polar_chart.dart`, `lib/screens/tool_config/configurators/polar_chart_configurator.dart`, `lib/services/tool_config_service.dart`, `lib/screens/tool_config_screen.dart`, `test/config/navigation_constants_test.dart` (new) |
