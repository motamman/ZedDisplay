...
- [ ] AIS: ensure data stays live both in the vessel list AND in the single-vessel detail view — verify both refresh on incoming AIS deltas (and own-vessel motion for distance/CPA/TCPA), not just on open
...
- [ ] Migrate off discontinued `flutter_markdown 0.7.7+1` → `flutter_markdown_plus` (drop-in replacement)
