...
- **Chart Plotter**: Full-featured chart plotter with native Flutter S-57 rendering
  ...
  - **Weather Routing**: Plan a weather-aware route from inside the chart plotter — boat + polar picker (filterable), per-leg `RoutePrecision`, per-waypoint arrival radius with approximate-mode halo, SSE-streamed progress while the solver runs, itinerary card with per-leg distance + time, Compose popover entry with `autoSubmit` for context jumps
    - **Editable & Re-Runnable** — Reload any route from the Recent tab and its start, end, and via points become live, draggable planning pins. Drag a pin, delete a via, hit Compute again under fresh weather. Tolerances persist in SI and round-trip from legacy display-unit blobs.
    - **Virtual START / END Cards** — When the router has to relocate your clicked endpoint to find clear water, the user's original point gets its own navigable card in the itinerary with a "Start/End adjusted X to clear water" banner (units honour your distance preference).
    - **Dateline-Safe** — Route polyline, waypoint decoration, snap bridges, and selection halo all unwrap into the world copy nearest the camera. Routes crossing ±180° paint as one continuous polyline at any pan.
